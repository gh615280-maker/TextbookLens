...
export function snapshotEpubRange(range: Range, sectionId: string, cfi: string): { text: string; sectionId: string; cfi: string; quote: TextQuote } | null {
  const text = range.toString();
  if (!text || !cfi) return null;
  const root = range.commonAncestorContainer.nodeType === Node.ELEMENT_NODE ? range.commonAncestorContainer as Element : range.commonAncestorContainer.parentElement;
  const sectionText = root?.ownerDocument?.body.textContent ?? text;
  const start = sectionText.indexOf(text);
  return { text, sectionId, cfi, quote: createTextQuote(sectionText, start < 0 ? 0 : [...sectionText.slice(0, start)].length, start < 0 ? [...text].length : [...sectionText.slice(0, start + text.length)].length) };
}
