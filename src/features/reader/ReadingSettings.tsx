...
export function ReadingSettings({
  ...
  const update = (field: keyof ReaderSettings, value: number | ReaderTheme) =>
  ...
  return (
    <section className="reader-settings" aria-label="阅读设置">
      <Select
        ...
        onSelectionChange={(value) => update('theme', value as ReaderTheme)}
      >
        <Label>主题</Label>
        <Button>
          <SelectValue />
        ...
        <Popover>
          <ListBox>
            {(['light', 'dark', 'system'] as const).map((theme) => (
              <ListBoxItem key={theme} id={theme}>
                {theme}
              </ListBoxItem>
            ))}
          </ListBox>
      ...
      {format === 'pdf' && (
        <Range
          ...
          max={3}
          step={0.1}
          onChange={(value) => update('pdfZoom', value)}
        />
      )}
    ...
    >
      ...
      <SliderTrack>
        {({ state }) => (
          <>
            <SliderThumb index={0} />
            <output>{state.getThumbValueLabel(0)}</output>
          </>
        )}
      </SliderTrack>
    </Slider>
