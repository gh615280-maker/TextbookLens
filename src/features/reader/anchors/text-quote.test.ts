...
describe('text quotes', () => {
  ...
  it('preserves emoji boundaries when creating and finding quotes', () => {
    ...
    expect(quote).toEqual({ exact: 'selected 中', prefix: 'before 😀 ', suffix: ' after' });
  ...
  it('returns a unique exact occurrence without requiring context', () => {
    expect(findTextQuote('changed prefix selected changed suffix', {
      exact: 'selected', prefix: 'old prefix', suffix: 'old suffix',
  ...
  it('uses both contexts to resolve repeated exact text', () => {
    ...
    expect(findTextQuote(text, { exact: 'target', prefix: 'before ', suffix: ' after' }))
  ...
  it('prefers a matching suffix before a matching prefix', () => {
    ...
    expect(findTextQuote(text, { exact: 'target', prefix: 'prefix ', suffix: ' suffix' }))
  ...
  it('rejects repeated text with equally reliable context', () => {
    ...
    expect(findTextQuote('same target tail and same target tail', quote)).toBeNull();
  });

  it('does not match altered text or reach beyond its supplied scope', () => {
    expect(findTextQuote('page one target', { exact: 'target', prefix: '', suffix: '' }))
      .toMatchObject({ confidence: 'exact_unique' });
    expect(findTextQuote('page one', { exact: 'target', prefix: '', suffix: '' })).toBeNull();
    expect(findTextQuote('targetx', { exact: 'target ', prefix: '', suffix: '' })).toBeNull();
  });

  it('round-trips quote ranges across representative Unicode text', () => {
    for (const text of ['ASCII target text', '中😀 target café', 'a\u0308 target']) {
