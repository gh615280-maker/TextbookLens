...
describe('ReaderLayout', () => {
  it('provides collapsible TOC and panel regions with accessible toolbar controls', async () => {
    ...
    expect(screen.getByRole('complementary', { name: '学习面板' })).toBeVisible();
    ...
    expect(screen.queryByRole('complementary', { name: '学习面板' })).not.toBeInTheDocument();
  });

  it('clears transient selection UI on Escape without moving focus for panel updates', async () => {
    const user = userEvent.setup();
    render(<ReaderLayout title="Fixture" initialSelectionActive panelContent="准备中" />);
    const toggle = screen.getByRole('button', { name: '折叠目录' });
    toggle.focus();

    await user.keyboard('{Escape}');

    expect(screen.queryByRole('status', { name: '已选择文本' })).not.toBeInTheDocument();
