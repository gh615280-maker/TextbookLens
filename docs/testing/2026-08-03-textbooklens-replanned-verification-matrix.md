...
| Phase 6  | three-language/Fluent/reader shell                 | BLOCKED | 2026-08-03 / product `8fc6245d66ec41e301f90feaf06320f721fe1404`; Windows 11-only planning baseline `9141c47` | Automated checkpoint gates and the 4-case focused regression PASS. Required durable Windows 11 x64 evidence at real 200% system display scaling plus high-contrast/opaque fallback remains unavailable; no Windows 10 compatibility claim is made.                                                          |
...
| B   | zh-CN/zh-TW/en                  | Phase 6/15       | Windows 11 locale/system scale     | BLOCKED | Three-language immediate switch and restart mock PASS in Playwright; durable Windows 11 evidence at real 200% system display scaling remains unavailable.                                                                   |
...
| O   | responsive/fullscreen           | Phase 6/12/15    | Windows 11, 200%, high contrast    | BLOCKED | Narrow/forced-colors/axe and F11/Esc/focus automation PASS; the Windows 11 host remained at its original 150% scale with high contrast off, so no durable real-200% or high-contrast/opaque-fallback artifact was produced. |
...
| clean Windows 11 x64 install                    | BLOCKED                | Windows 11 Pro build 22631 is available, but required real-200% Phase 6 evidence remains unavailable; the Phase 15 clean-install/release gate is not yet run. |
...
Required manual checkpoint status is **BLOCKED**, not PASS. The V1 support and release verification
baseline is Windows 11 x64 only; Windows 10 is outside V1 support and acceptance, and no compatibility
claim is made. A read-only check recorded the original host as Windows 11 Pro 23H2 build 22631 at
144 DPI (150% system display scaling) with high contrast off. Computer Use was available, but an
explicit launch of `SystemSettings.exe` exposed no targetable Settings window; recovery enumeration
also found no Settings window. The safety contract forbids guessing a window handle or using the
Windows key/Run dialog, so display and contrast settings were not changed and no restore operation was
needed. No UAC, sign-out, authentication, software install, VM setup, browser zoom, or emulation was
used, and no evidence directory was created. Phase 6 remains BLOCKED only on durable synthetic-content
Windows 11 evidence at real 200% system display scaling covering the reader shell, high-contrast/opaque
fallback, narrow readability, and fullscreen/Esc/focus behavior.
